...
function Airports() {
  ...
  const { loading, data } = useQuery(getAirports, {variables:{search:'south africa'},});
  ...
  return (
    <div className="container">
      <div className="row">
        {data && Object.values(data.airports).filter(ax => ax.imageUrl).map((airport, index) => (
          <div className="col-lg-3 col-md-4 col-6" key={index}>
            <div className="card mb-4 box-shadow d-block">
              <img
                className="card-img-top"
                alt="Thumbnail .get(100%x200)"
